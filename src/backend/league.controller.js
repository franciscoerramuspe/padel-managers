...
export async function updateMatchResult(req, res) {
  ...
  const homeTeamUpdate = {
    matches_played: homeTeamStanding.data.matches_played + 1,
    goals_for: homeTeamStanding.data.goals_for + home_sets,
    goals_against: homeTeamStanding.data.goals_against + away_sets
  };

  if (home_sets > away_sets) {
    homeTeamUpdate.wins = homeTeamStanding.data.wins + 1;
    homeTeamUpdate.points = homeTeamStanding.data.points + 3;
  } else if (home_sets === away_sets) {
    homeTeamUpdate.draws = homeTeamStanding.data.draws + 1;
    homeTeamUpdate.points = homeTeamStanding.data.points + 1;
  } else {
    homeTeamUpdate.losses = homeTeamStanding.data.losses + 1;
  }

  const awayTeamUpdate = {
    matches_played: awayTeamStanding.data.matches_played + 1,
    goals_for: awayTeamStanding.data.goals_for + away_sets,
    goals_against: awayTeamStanding.data.goals_against + home_sets
  };

  if (away_sets > home_sets) {
    awayTeamUpdate.wins = awayTeamStanding.data.wins + 1;
    awayTeamUpdate.points = awayTeamStanding.data.points + 3;
  } else if (away_sets === home_sets) {
    awayTeamUpdate.draws = awayTeamStanding.data.draws + 1;
    awayTeamUpdate.points = awayTeamStanding.data.points + 1;
  } else {
    awayTeamUpdate.losses = awayTeamStanding.data.losses + 1;
  }

  const { error: homeUpdateError } = await supabase
    .from('league_standings')
    .update(homeTeamUpdate)
    .eq('id', homeTeamStanding.data.id);

  const { error: awayUpdateError } = await supabase
    .from('league_standings')
    .update(awayTeamUpdate)
    .eq('id', awayTeamStanding.data.id);
  ...
  res.status(200).json({
    ...
    match: { ...match, home_sets, away_sets, status: 'completed' }
  });
}
